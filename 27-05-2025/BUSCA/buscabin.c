...
int main()
{
    ...
    if (posicao == -1)
        ...
        printf("Valor encontrado na posicao %d", posicao + 1); // soma 1 para mostrar posição começando em 1 (não em 0)
...
int PesqBin(int numeros[], int x)
{
    ...
    while (ini <= fim)
    {
        ...
        // Verifica se o elemento no meio é igual ao que estamos procurando
        if (numeros[meio] == x)
            return meio; // Achou! Retorna a posição

        // Se o número no meio é menor que o que buscamos, busca na metade da direita
        if (numeros[meio] < x)
            ini = meio + 1;

        // Se o número no meio é maior, busca na metade da esquerda
        if (numeros[meio] > x)
            fim = meio - 1;
    }
